...
class RustParser:
            ...
            if in_impl:
                ...
                if brace_depth > 0:
                    ...
                    if func_match:
                        ...
                        ret_type = func_match.group(3).strip() if func_match.group(3) else ""
    ...
    def _extract_call_edges(self, source: str, rel: str, graph: CodeGraph) -> None:
        ...
        keywords = {"if", "else", "for", "while", "loop", "match", "return",
                     "ref", "let", "const", "static", "type", "impl", "trait",
                     "struct", "enum", "fn", "use", "mod", "pub", "async", "await",
                     "do", "dyn", "self", "super", "crate", "where", "unsafe",
                     "mut", "move", "box", "break", "continue"}
        for m in call_pattern.finditer(source):
            caller_key = f"{rel}:{source[:m.start()].count(chr(10)) + 1}"
            callee = m.group(1)
            if callee in keywords or callee in ("Some", "None", "Ok", "Err", "Result", "Option"):
                continue
