...
class JavaScriptParser:
    ...
    def _extract_functions(self, source: str, rel: str, graph: CodeGraph) -> None:
        ...
        for m in func_pattern.finditer(source):
            ...
            is_async = (
                "async" in source[m.start() : m.start() + 50].split("function")[0]
            )
    ...
    def _extract_exports(self, source: str, rel: str, graph: CodeGraph) -> None:
        ...
        named_exports = re.findall(r"export\s+\{([^}]+)\}", source)
        ...
        default_match = re.search(
            r"export\s+default\s+(?:class|function|const|let|var)\s+(\w+)", source
        )
        if default_match:
    ...
    def _extract_imports(self, source: str, rel: str, graph: CodeGraph) -> None:
        ...
        side_effect = re.findall(r"import\s+['\"]([^'\"]+)['\"]", source)
    ...
    def _extract_variables(self, source: str, rel: str, graph: CodeGraph) -> None:
        ...
        existing_lines = {(sym.file, sym.line) for sym in graph.symbols.values()}

        var_pattern = re.compile(
            r"^(?:export\s+)?(?:const|let|var)\s+(\w+)\s*=", re.MULTILINE
        )
